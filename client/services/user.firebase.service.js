    ...
    .factory('userFirebaseService',["firebaseService", "$q", "$timeout",'$http', "$sessionStorage", "$firebaseObject",'appConfig','$localStorage','userService',
        ...
        return {
            getUserMembershipsSyncObj: function(userID){
                var response = { groupArray: [], userActivityStream: []};
                var self = this;
                ...
                response.userGroupMembershipRef.on('child_added', function(snapshot) {
                    $timeout(function() {
                        ...
                        response.groupArray.push({groupID: snapshot.key(), membershipType: snapshot.val()["membership-type"],
                            timestamp: snapshot.val()["timestamp"], groupData: groupSyncObj});
                    }, 1000);
            ...
            asyncCreateGroup: function( userID, groupObj ,loggedInUserGroupsList,formDataFlag){
                ...
                firebaseService.asyncCheckIfGroupExists(groupObj.groupID).then(function(response){
                    if(response.exists){
                        ...
                        firebaseService.getRefUserGroupMemberships().child(userID).child(groupObj.groupID).update({"membership-type": 1, timestamp: firebaseTimeStamp}, function(error1){
                            if(error1){
                                deferred.reject();
                            }
                            else {
                                firebaseService.getRefGroupMembers().child(groupObj.groupID).child(userID).set({"membership-type": 1, timestamp: firebaseTimeStamp}, function(error2){
                                    if(error2){
                                        deferred.reject();
                                    }
                                    else {
                            ...
                            if(membershipData){
                                ...
                                ref.child(userID).once("value", function(snap){
                                    ...
                                    if(alreadyPending){
                                        ...
                                        var request = {};
                                        request[userID] = {timestamp: firebaseTimeStamp, message: message};
                                        ref.update(request, function(error){
                                            if(error){
                                                ...
                                                ref.child(userID).once("value", function(snapshot){
                                                    ...
                                                    var requestByUser = {};
                                                    requestByUser[groupID] = {timestamp: timestamp};
                                ...
                                ref.set({
                                    ...
                                    if (error) {
                                        ...
                                        ref.once("value", function (snapshot) {
                                            ...
                                            refByUser.set({timestamp: timestamp}, function (error) {
                                                if (error) {
                                                    //roll back previous may be
                                                    deferred.reject("Server Error, please try again");
                                                }
