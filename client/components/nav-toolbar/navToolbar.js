(function() {
  ...
  .controller('NavToolbarController', ['activityStreamService',
    'ProgressReportService', '$mdSidenav', '$mdDialog', '$mdMedia',
    '$interval', '$q', '$rootScope', 'soundService', 'messageService',
    '$timeout', '$firebaseArray', 'navToolbarService', 'authService',
    '$firebaseObject', 'firebaseService', 'userService', '$state',
    '$location', 'checkinService',
    function(activityStreamService, ProgressReportService, $mdSidenav,
      $mdDialog, $mdMedia, $interval, $q, $rootScope, soundService,
      messageService, $timeout, $firebaseArray, navToolbarService,
      authService, $firebaseObject, firebaseService, userService, $state,
      $location, checkinService) {
      /*private variables*/
      ...
      function checkingHasPolicy(groupID, subgroupID, cb) {
        firebaseService.getRefSubGroupsNames().child(groupID).child(
          subgroupID).once('value', function(snapshot) {
          self.subGroupHasPolicy = (snapshot.val() && snapshot.val().hasPolicy) ?
            snapshot.val().hasPolicy : false;
          // console.log('subGroupHasPolicy', self.subGroupHasPolicy);

          if (self.subGroupHasPolicy) {
            firebaseService.getRefPolicies().child(groupID).child(
              snapshot.val().policyID).once('value', function(
              policy) {
      ...
      checkinService.getRefSubgroupCheckinCurrentByUser().child(userID).on(
        'value',
        function(snapshot, prevChildKey) {
          ...
          if (snapshot.val() && snapshot.val().type == 2) {
            ...
            firebaseService.getRefGroupsNames().child(self.showUrlObj.groupID)
              .child('title').once('value', function(snapshot) {
                self.showUrlObj.groupTitle = snapshot.val();
              });
            firebaseService.getRefSubGroupsNames().child(self.showUrlObj
              .groupID).child(self.showUrlObj.subgroupID).child(
              'title').once('value', function(snapshot) {
      ...
      self.groups = $firebaseArray(firebaseService.getRefUserSubGroupMemberships()
        .child(userID));
      ...
      function updateStatus(group, checkoutFlag, event) {
        ...
        if (group) {
          groupObj = {
            ...
            userId: userID,
            subgroupTitle: group.subgroupTitle
          };
        } else {
          groupObj = {
        ...
        checkinService.ChekinUpdateSatatus(groupObj, userID, checkoutFlag,
          function(result, msg, isSubmitted, groupObject) {
            if (result) {
              ...
              if (checkoutFlag) {
                ...
                var type = 'subgroup';
                var targetinfo = {
                  id: groupObj.subgroupId,
                  url: groupObj.groupId + '/' + groupObj.subgroupId,
                  title: self.showUrlObj.subgroupTitle,
                  type: 'subgroup-checkout'
                };
                var area = {
                  type: 'subgroup-checkout'
                };
                ...
                activityStreamService.activityStream(type, targetinfo,
                  area, group_id, memberuserID, _object);
                ...
                if (isSubmitted) {
                  ...
                  self.dailyProgressReport = ProgressReportService.getSingleSubGroupReport(
                    userObj, groupObj.groupId, groupObj.subgroupId);
                ...
                var _targetinfo = {
                  id: group.subgroupId,
                  url: group.pId + '/' + group.subgroupId,
                  title: group.subgroupTitle,
                  type: 'subgroup-checkin'
                };
                var _area = {
                  type: 'subgroup-checkin'
                };
                ...
                var __object = null;
                //for group activity record
                activityStreamService.activityStream(_type, _targetinfo,
                  _area, _group_id, _memberuserID, __object);
                //for subgroup activity stream record -- END --
      ...
      this.submitReport = function() {
        ...
        self.switchMsg = false;
        self.isDailyProgessSubmit = false;
        $state.go('user.group.subgroup-progressreport', {
          groupID: self.isDailyProgessgroupID,
          subgroupID: self.isDailyProgesssubgroupID,
          u: true
        });
      };
      ...
      function checkinPolicy(callback) {
        if (self.subGroupPolicy.locationBased) { //checking if location Based
          ...
          var distance = self.CalculateDistance(self.subGroupPolicy.location
            .lat, self.subGroupPolicy.location.lng, self.currentLocation
            .lat, self.currentLocation.lng, 'K');
          ...
          if ((distance * 1000) > self.subGroupPolicy.location.radius) { //checking lcoation radius
            ...
            self.switchCheckIn = false;
            messageService.showFailure(
              'Current Location does not near to the Team Location');
            return false;
      ...
      function checkinTimeBased(callback) {
        ...
        if (self.subGroupPolicy.timeBased) {
          ...
          if (self.subGroupPolicy.schedule[Schduleday] && self.subGroupPolicy
            .schedule[Schduleday][today.getHours()]) {
      ...
      function updateHelper(group, checkoutFlag, event, cb) {
        ...
        //add/update in firebase...
        checkinService.saveFirebaseCheckInOut(groupObj, checkoutFlag,
          self.currentLocation, cb);
        //updateFirebase(groupObj, checkoutFlag, cb);

      } //updateHelper
      function updateStatusHelper(groupID, subgroupID, userID,
        checkoutFlag) {
        checkinService.getCurrentLocation().then(function(location) {
          if (location) {
          ...
          checkinService.updateUserStatusBySubGroup(groupID,
              subgroupID, userID, self.checkinObj.newStatus, self.definedSubGroupLocations,
              null)
            .then(function(res) {
              $timeout(function() {
      ...
      function updateFirebase(groupObj, checkoutFlag, cb) { //on checkout checkoutFlag is true, on checkin checkoutFlag is false
        ...
        var checkinMessage = (checkoutFlag) ? "Checked-out" :
          "Checked-in";
        ...
        refGroup.child(groupObj.groupId).child('members-checked-in').child(
          'count').once('value', function(snapshot) {
          multipath["groups/" + groupObj.groupId +
            "/members-checked-in/count"] = (checkoutFlag) ? (
            snapshot.val() - 1) : (snapshot.val() + 1);
      ...
      this.checkinClick = function(event) {
        ...
        self.groups.forEach(function(group, groupId) {
          ...
          for (var i in group) {
            if (['$priority', '$id'].indexOf(i) == -1 && typeof group[
                i] === 'object') {
              var temp = {};
              temp.pId = group.$id; // group Name == pId
              temp.subgroupId = i;
              temp.subgroupTitle = checkinService.getSubGroupTitle(
                group.$id, i);
              temp.data = group[i];
      ...
      function showSubGroup(group, pId) {
        self.subgroups = [];
        for (var i in group) {
          if (['$priority', '$id'].indexOf(i) == -1 && typeof group[i] ===
            'object') {
            var temp = {};
            temp.pId = group.$id; // group Name == pId
            temp.subgroupId = i;
      ...
      function queryGroups() {
        if (self.search) {
          var filteredGroupsNamesRef = firebaseService.getRefUserSubGroupMemberships()
            .child(userID)
            .orderByKey()
            .startAt(self.search)
            .endAt(self.search + '~');

          self.filteredGroups = Firebase.getAsArray(
            filteredGroupsNamesRef);
          ...
          // self.filteredGroups = [];
          self.filteredGroups = Firebase.getAsArray(firebaseService.getRefUserSubGroupMemberships()
            .child(userID));
        }
      ...
      function PersonalSetting() {
        ...
        $state.go('user.personal-settings', {
          userID: userService.getCurrentUser().userID
        })
      }
      ...
      this.updateProgressReport = function() {
        ...
        ProgressReportService.updateReport(self.dailyProgressReport[0],
          function(result) {
            if (result) {
              ...
              messageService.showFailure(
                'Progress Report Update Failure!');
            }
          });
      };
