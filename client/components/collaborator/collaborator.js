...
(function() {
  ...
  function collaboratorFunction(firebaseService, ref, $firebaseArray, FileSaver, Blob, groupService, $stateParams, userService, dataService, messageService, $timeout, $scope, $state, $firebaseObject, $rootScope, CollaboratorService, $q) {
      ...
      obj.$save();
        ...
        that.checkboxClicked = function (userStatus,user) {
          if(userStatus){
            ...
            user.id == that.createdBy.userID ? userStatus = 1 : userStatus = 0;
          }
    ...
    function getUserPermissions(cb) {
      var defered = $q.defer();
      ...
      if (that.subgroupID)
        ...
        firepadRef = firepadPermissions.child("firepad-groups-access/" + that.groupID + "/" + that.subgroupID + "/" + $stateParams.docID);
